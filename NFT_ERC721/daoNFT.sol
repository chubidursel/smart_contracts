...
contract GuideDAOAccessToken is IERC721Metadata {
    ...
    function burn(address from, uint256 tokenId) public isAdmin {
        _transferFrom(from, address(this), tokenId);
    }
    ...
    function getAddressNFTId(address user) public view returns (int256) {
        require(currentIdToMint > 0, "Nothing to search");
        if (balanceOf(user) == 0) {
            return -1;
        }
        for (uint256 i = 0; i < currentIdToMint; i++) {
            if (ownerOf(i) == user) {
                return int256(i);
            }
        }
